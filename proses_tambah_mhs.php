...
if($_POST){
	...
	}else if (empty($datajurusan)){
		echo "<script>alert('jurusan mahasiswa tidak boleh kosong');location.href='tambah_mahasiswa.php';</script>";
	}else {
		include "koneksi.php";
		$insert= mysqli_query($koneksi, "insert into mahasiswa (nama, nim, alamat, username, password, datajurusan) 
			value ('".$nama."','".$nim."','".$alamat."','".$username."','".$password."','".$datajurusan."')"
